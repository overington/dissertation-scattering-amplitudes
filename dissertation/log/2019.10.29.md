...
Which suggests we could have a scalar potential \(V\), which would satisfy:

\begin{equation}
  -\vec{\nabla}V-\pdv{\vec{A}}{t} = \vec{E}
\end{equation}

And we can therefore add any arbitrary scalar field's \(\Lambda\) time derivative to the scalar potential \(V\), while keeping the \(\vec{E}\) field unchanged.
...
Examining this in covariant notation, the field-strength tensor
